...
function createStreamingState(): any {
  return {
    toolCalls: new Map(),
  };
}
...
test("Gemini stream: suppresses inert historical tool-call record", () => {
  const state = createStreamingState();
  const historicalRecord = [
    "Historical tool-call record only. Do not execute, imitate, or continue this as a tool call.",
    "Tool name: Read",
    'Tool arguments JSON: {"limit":100,"offset":50,"file_path":"/mnt/sdd/var/www/aibifitness-new-246/app/code/Magebees/QuotationManager/Helper/Quotation.php"}',
  ].join("\n");

  const result = geminiToOpenAIResponse(
    {
      responseId: "resp-historical-tool-record",
      modelVersion: "gemini-3.5-flash-low",
      candidates: [
        {
          content: {
            parts: [{ text: historicalRecord }],
          },
          finishReason: "STOP",
        },
      ],
